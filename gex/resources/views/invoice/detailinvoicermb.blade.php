        ...
        <table class="table table-borderless detail-table no-margin">
            <tbody>
                ...
                <tr>
                    <td>E-FAKTUR</td>
                    <td>:</td>
                    <td>{{ $invoice->efaktur }}</td>
                </tr>
            </tbody>
        </table>
    </div>
    @php
        $customer = App\MasterCustomer::find($receivables->rmb_marketing->customer_id);
        $term = App\MasterTerm::find($receivables->rmb_marketing->term_id);
    @endphp
    <div class="col-md-4">
        <table class="table table-borderless detail-table no-margin">
            <tbody>
                ...
                <tr>
                    ...
                    <td>:</td>
                    <td>
                        @if($receivables->rmb_tax == 1)
                            PPN 1
                        @elseif($receivables->rmb_tax == 2)
                            PPN 2
                        @else
                            NON PPN
                        @endif
...
<div class="row">
    <div class="col-md-12">
        <table class="table table-bordered table-body-condensed table-striped no-margin">
            ...
            <tbody>
                ...
                @foreach($charges as $charge)
                    @php
                        $doc = App\MasterDocument::find($charge->rmb_document_id);
                        $unit = App\MasterUnit::find($charge->rmb_unit_id);
                        $sum += $charge->rmb_total;
                    @endphp
                <tr>
                    <td>{{ $no }}</td>
                    <td>{{ $doc->name }}</td>
                    <td class="text-center">{{ $charge->rmb_quantity }} x {{ $unit->name }}</td>
                    <td class="text-center">@if($charge->rmb_currency == 1) IDR @else USD @endif</td>
                    <td class="text-right">{{ number_format($charge->rmb_price) }}</td>
                    <td class="text-right">
                        {{-- @if($max == $charge->rmb_total)
                        @if($charge->rmb_tax == 1)
                            {{ number_format($charge->rmb_total - $tot * 1/100) }}
                        @elseif($charge->rmb_tax == 2)
                            {{ number_format($charge->rmb_total - $tot/1.01 * 1/100) }}
                        @else
                            {{ number_format($charge->rmb_total - $tot/1.01 * 1/100) }}
                        @endif
                    @else
                            {{ number_format($charge->rmb_total) }}
                    @endif --}}
                    {{ number_format($charge->rmb_total) }}
                    </td>
                </tr>
        ...
        <div class="text-right">
            ...
            @endif
            <a class="btn btn-primary" href="{{ route('invoice.printpdfrmb', $invoice->id) }}" target="blank"><i class="fa fa-print fa-fw"></i> PRINT</a>
        </div>
    </div>
</div>
