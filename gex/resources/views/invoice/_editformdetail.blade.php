        ...
        <div class="form-group">
            ...
            <div class="col-sm-8">
                <div class="input-group">
				    <input class="form-control input-sm datepicker" name="date" placeholder="Create Date" type="text" value="{{date('d m Y', strtotime($invoice->tanggal))}}">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-8">
                ...
                <select name="bank_id" class="form-control input-sm">
                    ...
                    @endphp
                        <option value="{{ $invoice->bank_id }}">{{ $bank->name }}</option>
                    @foreach(App\MasterBank::all() as $b)
        ...
        <div class="form-group">
            ...
            <div class="col-sm-8">
                @foreach($references as $ref)
                    ...
                    @endphp
                    <input type="checkbox" name="ref[]" value="{{ $doc->name.'-'.$ref->ref_no }}" checked="true"> {{ $doc->name }} - {{ strtoupper($ref->ref_no) }} <br />
                @endforeach
            </div>
        </div>
    </div>
    <div class="col-sm-5 col-md-5 col-md-offset-1">
        <div class="form-group">
            ...
            <label class="control-label col-sm-6 col-md-7 label-right">
                @php
                    $char = App\Receivable::find($charges[0]);
                    $customer_id = App\MasterCustomer::find($jobsheet->customer_id);
                    $cust = App\MasterCustomer::find($char->rec_marketing->customer_id);
                    $term = App\MasterTerm::find($char->rec_marketing->term_id);
                @endphp
                {{ $customer_id->name }}
...
<div class="table-responsive">
    <table class="table table-bordered table-body-condensed table-striped no-margin">
        ...
        @foreach($charges as $charge)
            <?php
                $cha = App\Receivable::find($charge);
                $doc = App\MasterDocument::find($cha->rec_document_id);
                $unit = App\MasterUnit::find($cha->rec_unit_id);
                $sum += $cha->rec_total;
            ?>
            <input type="hidden" name="rec_id[]" value="{{ $charge }}">
            <tr>
                <td>{{ $no }}</td>
                <td>{{ $doc->name }}</td>
                <td class="text-center">{{ $cha->rec_quantity }} x {{ $unit->name }}</td>
                <td class="text-center">
                    @if($cha->rec_currency == 1)
                        IDR
                    @else
                        USD
                    @endif
                </td>
                <td class="text-right">{{ number_format($cha->rec_price) }}</td>
                <td class="text-right">
                    @if($max == $cha->rec_total)
                        @if($cha->rec_tax == 1)
                            {{ number_format($cha->rec_total) }}
                        @elseif($cha->rec_tax == 2)
                            {{ number_format($cha->rec_total - $tot/1.01 * 1/100) }}
                        @else
                            {{ number_format($cha->rec_total - $tot/1.01 * 1/100) }}
                        @endif
                    @else
