        ...
        <div class="col-sm-4">
            <table class="table table-borderless table-condensed detail-table no-margin">
                ...
                <tr>
                    <td>PARTY/MEAS</td>
                    <td>:</td>
                    <td>{{ $jobsheet->partymeas }} x {{ $jobsheet->party_unit->name or '' }}</td>
                </tr>
                <tr>
                    <td>REMARKS</td>
                    <td>:</td>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-condensed table-striped invoice-table no-margin">
                    ...
                    <tbody class="tbody">
                        <input type="hidden">
                        @php $no=1; @endphp
                        @foreach($charges as $receive)
                                @php
                                    $rec_bill = App\MasterCustomer::find($receive->rec_marketing->customer_id);
                                    $rec_term = App\MasterTerm::find($receive->rec_marketing->term_id);
                                    $rec_unit = App\MasterUnit::find($receive->rec_unit_id);
                                    $rec_doc  = App\MasterDocument::find($receive->rec_document_id);
                                    $rec_chg  = $receive->rec_charge_type;
                                    $rec_tax  = $receive->rec_tax;
                                    $rec_cur  = $receive->rec_currency;
                                    $rec_ttl  = $receive->rec_price * $receive->rec_quantity;
                                @endphp
                        <tr>
                        <input type="hidden" name="total[]" value="{{ $receive->rec_total }}">
                            <td>{{ $no }}</td>
                            <td>
                                @if($invoice->id == $receive->rec_invoice_id)
                                <label class="fancy-checkbox">
                                    <!-- <input  type="checkbox" name="charges" class="check-req" value="{{ $receive->id }}" required autofocus="">
                                     -->
                                     {{ Form::checkbox('charges[]', $receive->id, true, ['class' => 'check-req']) }}
                                     <span></span>
                                </label>
                                @endif
                            </td> 
                            <td>
                                        @if($rec_chg == 1)
                                            CHARGES ( {{ $rec_doc->name }} )
                                        @elseif($rec_chg == 2)
                                            FREIGHT CHARGES ( {{ $rec_doc->name }} )
                                        @else
                                            FREIGHT CHARGES COLLECT ( {{ $rec_doc->name }} )
                                        @endif
                            </td>
                            <td>{{ $rec_bill->name }}</td>
                            <td>{{ $rec_term->name }}</td>
                ...
                <table class="table table-bordered table-condensed table-striped invoice-table no-margin">
                    ...
                    <tbody class="tbody">
                        ...
                        <input type="hidden" name="total[]" value="{{ $rmb->rmb_total }}">
                            <td>{{ $no }}</td>
                            <td>
                                @if($invoice->id == $rmb->rmb_invoice_id)
                                <label class="fancy-checkbox">
                                    <!-- <input  type="checkbox" name="charges" class="check-req" value="{{ $rmb->id }}" required autofocus="">
                                     -->
                                     {{ Form::checkbox('charges[]', $rmb->id, true, ['class' => 'check-req']) }}
                                <span></span>
                                </label>
                                @else
                                <label class="fancy-checkbox">
