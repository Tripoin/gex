										        	...
										        	@foreach($invoicerec->sortByDesc('id') as $rec)
											            ...
											            @endphp
												         <tr>
												            ...
												            <td>
												            		<a href="{{ route('invoice.receivable.show', [$job->id,$rec->id]) }}"> {{ $rec->code }} </a>
												            </td>  
												            ...
												            <td class="text-center">
												            	{{-- @if($rec->status!=1 && $rec->status!=2)
													            	<a href="{{ route('invoice.edit', [$rec->id, $rec->type]) }}" class="btn btn-warning btn-sm"><span class="glyphicon glyphicon-pencil"></span></a>
													            @else
													            	<a href="#" class="btn btn-warning btn-sm" disabled><span class="glyphicon glyphicon-pencil"></span></a>
													            @endif --}}
													            @if($rec->status==0)
													            	<a href="{{ route('invoice.printpdf', $rec->id) }}" target="_blank" class="btn btn-primary btn-sm"><i class="fa fa-print"></i></a>
													            @else
