...
<head>
    ...
    <title>Invoice Receivable</title>
    <style>
     body {
        font-family: "Calibri";
        width: 100%;
...
<br><br>
    ...
    @php
        ...
        $rece = App\Receivable::where('rec_invoice_id',$invoice->id)->first();
        $term = App\MasterTerm::find($rece->rec_marketing->term_id);
    ...
    <hr>
        ...
        @foreach($receivable as $rec)
            @php
                $doc = App\MasterDocument::find($rec['rec_document_id']);
                $unit = App\MasterUnit::find($rec['rec_unit_id']);
                $sum += $rec['rec_total'];
            @endphp
          <tr>
            <td width="30%">{{ $doc->name }}</td>
            <td width="4%">{{ $rec['rec_quantity'] }}</td>
            <td width="2%">
            ...
            <td width="10%" class="center">
                @if($rec['rec_currency'] == 2)
                    {{ number_format($rec['rec_price'],2) }}
                @endif
            </td>
            <td width="10%" class="center">
                @if($rec['rec_currency'] == 1)
                    {{ number_format($rec['rec_price'],2) }}
                @endif
            </td>
            <td width="5%"></td>
            <td width="10%" class="center">
                @if($rec['rec_currency'] == 2)
                    @if($max == $rec['rec_total'])
                        @if($rec['rec_tax'] == 1)
                            {{ number_format($rec['rec_total'],2) }}
                        @elseif($rec['rec_tax'] == 2)
                            {{ number_format($rec['rec_total'] - $tot/1.01 * 1/100,2) }}
                        @else
                            {{ number_format($rec['rec_total'] - $tot/1.01 * 1/100,2) }}
                        @endif
                    @else
