...
<div class="modal fade" id="modal-invoice">
    <div class="modal-dialog modal-big">
        <div class="modal-content">
            ...
            <form action="{{url('/invoice/reimbursement/store')}}" method="POST" role="form" class="form-horizontal">
                {{ csrf_field() }}
                <div class="modal-body">
                    <div class="row">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-8">
                                    <div class="input-group">
									    <input class="form-control input-sm datepicker" placeholder="Create Date" type="text" name="date">
                        ...
                        <div class="col-sm-5 col-md-5 col-md-offset-1">
                            <div class="form-group">
                                <label class="control-label col-sm-4">CUSTOMER</label>
                                <input type="hidden" name="id_reimbursement" id="id_reimbursement_modal">
                        ...
                        <table class="table table-bordered table-body-condensed table-striped no-margin">
                            ...
                            <tbody id="tbody_modal">
                                ...
                                <tr>
                                    ...
                                    <td class="text-right"><strong id="ammount_due_modal">3,300.00</strong></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
$(function(){
    ...
    var table = $('#myTable').DataTable({
        ...
        "ajax":{
            type : "POST",
            url : "/invoice/jobsheet/get/reimbursement"
        } ,
        "columnDefs": [
            { 
                targets: 0, 
    ...
    $('#myTable').on('click', '.detailJobSheet', function(event){
        ...
        if(arrayTabs.indexOf(jobId) == -1){
            ...
            //load form pd panel
            $.get(`{{ url('/invoice/jobsheet/get/reimbursement/detail') }}`, {id: jobId}, function(result){
    ...
    $('.tab-content').on('click','.btn-modal-invoice', function(e){
        var id = $(this).closest("form").find('.hidden-id-rmb').val();
      
        if(id==""){
            alert ('Anda Belum Memilih Charges Yang Akan di Buat Invoicenya!');
            return e.stopPropagation();
        }
        $.post("/invoice/check/reimbursement",{"id": id}, function(data, status){
            if(status == "success"){
                var errorlist = '';
                ...
                if(errorlist != ''){
                    ...
                    var doc_modal ='';
                    var id_rmb = new Array;
                    for(var i=0; i<data['rmb'].length; i++){
                        id_rmb.push(data['rmb'][i]['id']);
                    }
                    $("#id_reimbursement_modal").val(id_rmb);
                    ...
                    var value = 0;
                    for(var i=0; i<data['rmb'].length; i++){
                        var currency = (data['rmb'][i]['currency'] == 1) ? "IDR":"USD";
                        var total = data['rmb'][i]['price']*data['rmb'][i]['qty'];
                        total = total.toFixed(2);
                        var price = data['rmb'][i]['price'];
                        price = price.toFixed(2);
                        rmb += `<tr class="list_rmb">
                                <td>${i+1}</td>
                                <td>${data['rmb'][i]['charge']['name']}</td>
                                <td class="text-center">${data['rmb'][i]['qty']} x ${data['rmb'][i]['unit']['name']}</td>
                                <td class="text-center">${currency}</td>
                                <td class="text-right">${price}</td>
                                <td class="text-right">${total}</td>
                            </tr>`;
                        value += data['rmb'][i]['price']*data['rmb'][i]['qty'];
                    }
                    $("#tbody_modal").find('.list_rmb').remove()
                    $("#tbody_modal").prepend(rmb);
                
                    $("#ammount_due_modal").text(value);
    ...
    $('.tab-content').on('change','.check-req', function() {
        ...
        $(this).closest("tbody").find('.hidden-id-rmb').val(id_check);
    });
