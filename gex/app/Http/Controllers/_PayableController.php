...
//                ->whereRaw("((select COUNT(id) from requests where jobsheet_id=jobsheets.id) != 0 or (select COUNT(id) from invoices where jobsheet_id=jobsheets.id) != 0)")
    ...
    public function jobsheet_createnew(Request $request, Datatables $datatables) {
        if (Auth::user()->role == 'payable' || Auth::user()->role == 'admin') {
            $jobsheets = JobSheet::where('jobsheets.status', ['completed', 'uncompleted'])
                    ->whereRaw("((select COUNT(id) from requests where jobsheet_id=jobsheets.id) = 0 or (select COUNT(id) from invoices where jobsheet_id=jobsheets.id) = 0)")
//                    ->orWhere("select COUNT(id) from invoices where jobsheet_id=jobsheets.id","<",0)
                    ->get();
        } else {
            $jobsheets = JobSheet::where('jobsheets.status', ['completed', 'uncompleted'])
                    ->get();
        ...
        return view('jobsheet.index_createnew', compact('jobsheets'));
    }
    ...
    public function jobsheet_edit_createnew(Request $request, Datatables $datatables, $id) {
        $jobsheet = JobSheet::find($id);
